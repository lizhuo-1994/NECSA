...
from tianshou.data import Collector, ReplayBuffer, VectorReplayBuffer, NECSA_Adv_Collector
...
def test_td3(args=get_args()):
    ...
    print(NECSA_DICT)
    train_collector = NECSA_Adv_Collector(policy, train_envs, buffer, exploration_noise=True, NECSA_DICT = NECSA_DICT)
    test_collector = Collector(policy, test_envs)
    train_collector.collect(n_step=args.start_timesteps, random=True)

    # log
    now = datetime.datetime.now().strftime("%y%m%d-%H%M%S")
    args.algo_name = "necsa_adv_td3"
