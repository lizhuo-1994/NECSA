...
class NECSA_Collector_ATARI(object):
    ...
    def collect(
        ...
        feature_net = getattr(self, "model")
        ...
        while True:
            ...
            # print("obs:", self.data.obs.shape)
            # print("observation:", observation.shape)
            obs_feature, _ = feature_net(observation)
            # print("obs_feature:", obs_feature.shape)
            obs_feature = torch.flatten(obs_feature)
            # print("obs_feature:", obs_feature.shape)
            sigmoid_net = torch.nn.Sigmoid()
            obs_feature = sigmoid_net(obs_feature)
            # print("obs_feature:", obs_feature.shape)
            reward = rew[0]
            done_env = done[0]
            ...
            if done:
                self.reward_list = self.abstracter.reward_shaping(np.array(list(state_action_list)), np.array(self.reward_list))
            ...
            if done:
                print("ep_idx[0]",ep_idx[0])
                print("ep_len[0]:",ep_len[0])
                print("buffer.rew:", np.array(self.buffer.rew[ep_idx[0]: ep_idx[0] + ep_len[0]]).shape)
                print("ep_reward:", np.array(self.ep_reward).shape)
